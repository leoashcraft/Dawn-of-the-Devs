...
/**
 * Set an error flash message, hiding error details in production.
 */
function flashError(req, productionMsg, err) {
  const message = config.IS_PRODUCTION ? productionMsg : `${productionMsg.replace(/\.$/, '')}: ${err.message}`;
  req.session.flash = { type: 'error', message };
}

module.exports = { commonLocals, requireAuth, requireAdmin, flashError };
